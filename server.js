...
app.post('/api/create-checkout-session', earnLimiter, async (req, res) => {
    ...
    if (!stripeKey) {
        return res.status(500).json({ success: false, error: 'Chybí STRIPE_SECRET_KEY v proměnných prostředí.' });
    }
    ...
    try {
        const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
                price_data: {
                    currency: 'czk',
                    product_data: { name: selectedVip.name },
                    unit_amount: selectedVip.amount,
                },
                quantity: 1,
            }],
            mode: 'payment',
            success_url: 'https://www.neonpaypulse.com/dashboard.html?payment=success&session_id={CHECKOUT_SESSION_ID}',
            cancel_url: 'https://www.neonpaypulse.com/dashboard.html?payment=cancelled',
        });
...
app.get('/api/verify-checkout-session', async (req, res) => {
    ...
    if (!session_id) {
        return res.json({ success: false, error: 'Chybí session_id' });
    }

    const stripeKey = process.env.STRIPE_SECRET_KEY || process.env.STRIPE_SK || '';
    if (!stripeKey) {
        return res.json({ success: false, error: 'Chybí klíč.' });
    }
    ...
    try {
        ...
        if (session && session.payment_status === 'paid') {
            await db.execute({
                sql: `UPDATE users SET hasBooster = 1 WHERE username = ?`,
                args: [req.session.username]
            });
            return res.json({ success: true, message: 'VIP úspěšně aktivováno!' });
        }
        res.json({ success: false, error: 'Platba nebyla dokončena.' });
    } catch (err) {
        console.error('Chyba při ověřování platby:', err);
        res.json({ success: false, error: 'Chyba při ověřování platby.' });
    }
});
